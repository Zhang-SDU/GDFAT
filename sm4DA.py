# !/usr/bin/env python3
# -*- coding: utf-8 -*-

"""
@File : sm4DA.py
@Author : A lover
@Email : 1146918446@qq.com
@Time : 2023/4/13 18:48
@Software: PyCharm
@Purpose:Recover the key by performing a differential analysis of the faulty ciphers and the correct cipher
"""

from enum import Enum

# Possible fault states
FaultStatus = Enum('FaultStatus', 'Crash Loop NoFault MinorFault MajorFault WrongFault GoodFault ')

blocksize = 16
# function of xor
xor = lambda a, b: list(map(lambda x, y: x ^ y, a, b))
# function of Rotation left
rotl = lambda x, n: ((x << n) & 0xffffffff) | ((x >> (32 - n)) & 0xffffffff)
# Four-byte list conversion to big-end 32bit numbers
get_uint32_be = lambda key_data: ((key_data[0] << 24) | (key_data[1] << 16) | (key_data[2] << 8) | (key_data[3]))
# Four-byte list conversion to small-end 32bit numbers
get_uint32_le = lambda key_data: ((key_data[3] << 24) | (key_data[2] << 16) | (key_data[1] << 8) | (key_data[0]))
# 32bit numbers conversion to big-end four-byte list
put_uint32_be = lambda n: [((n >> 24) & 0xff), ((n >> 16) & 0xff), ((n >> 8) & 0xff), ((n) & 0xff)]
# 32bit numbers conversion to small-end four-byte list
put_uint32_le = lambda n: [((n) & 0xff), ((n >> 8) & 0xff), ((n >> 16) & 0xff), ((n >> 24) & 0xff)]
# The inverse of the linear transformation L
l_inv = lambda c: c ^ rotl(c, 2) ^ rotl(c, 4) ^ rotl(c, 8) ^ rotl(c, 12) ^ rotl(c, 14) ^ rotl(c, 16) ^ rotl(c,
                                                                                                            18) ^ rotl(
    c, 22) ^ rotl(c, 24) ^ rotl(c, 30)
# Convert int numbers to bytes
int2bytes = lambda state, size: (state).to_bytes(size, byteorder='big', signed=False)
# Byte conversion to int numbers
bytes2int = lambda state: int.from_bytes(state, 'big', signed=False)
# Get single byte
singleState = lambda a, index: (a >> (index * 8)) & 0xff
# Get 32-bit State from 128-bit
getSlices = lambda block: [(block >> (32 * i) & 0xffffffff) for i in range(0, 4)]
# 16-byte list conversion to four big-end 32-bits state
byte2slices = lambda state: [get_uint32_be(state[i * 4: (i + 1) * 4]) for i in range(4)]
# Determining active differential bytes
find_candidate_index = lambda diff: [i for i in range(4, len(diff)) if diff[i] != b'\x00'][0] % 4

# Sbox of SM4
SM4_BOXES_TABLE = [
    ,
]


# Whether additional restrictions are met in satisfying the valid cipher mode
def check_diff(diffmap, n):
    for i in range(n - 1):
        if diffmap[i] != i:
            return False
    return True


def gen_diff_table():
    """
    Generate the differential table.
    :return: a differential table contains all the x satisfying S(x) ^ S(x ^ α) = β
    """
    IN_Table = [[[] for i in range(2 ** 8)] for j in range(2 ** 8)]
    for diff_in in range(1, 2 ** 8):
        for x in range(2 ** 8):
            diff_out = SM4_BOXES_TABLE[x] ^ SM4_BOXES_TABLE[diff_in ^ x]
            IN_Table[diff_in][diff_out].append(x)
    return IN_Table


IN_Table = gen_diff_table()


def f_function(x0, x1, x2, x3, rk):
    """
    SM4 Round functions F
    :param x0: 32 bits unsigned value
    :param x1: 32 bits unsigned value
    :param x2: 32 bits unsigned value
    :param x3: 32 bits unsigned value
    :param rk: 32 bits unsigned value
    :return: c 32 bits unsigned value
    """
    ka = x1 ^ x2 ^ x3 ^ rk
    b = [0, 0, 0, 0]
    a = put_uint32_be(ka)
    b[0] = SM4_BOXES_TABLE[a[0]]
    b[1] = SM4_BOXES_TABLE[a[1]]
    b[2] = SM4_BOXES_TABLE[a[2]]
    b[3] = SM4_BOXES_TABLE[a[3]]
    bb = get_uint32_be(b[0:4])
    c = bb ^ (rotl(bb, 2)) ^ (rotl(bb, 10)) ^ (rotl(bb, 18)) ^ (rotl(bb, 24))
    return (x0 ^ c)


def rewind(state, lastroundkeys=None):
    """
    rewind SM4 round according to roundkey
    :param state: SM4 state, as 16 bytes
    :param lastroundkeys: a list of 32-bit words round keys for the rounds to rewind.
    :return: SM4 Intermediate state
    """
    output = []
    ulbuf = [0] * 36
    ulbuf[0:4] = byte2slices(state)

    if lastroundkeys == None:
        return state

    round_num = len(lastroundkeys)

    for idx in range(round_num):
        ulbuf[idx + 4] = f_function(ulbuf[idx], ulbuf[idx + 1], ulbuf[idx + 2], ulbuf[idx + 3], lastroundkeys[idx])

    output += put_uint32_be(ulbuf[round_num])
    output += put_uint32_be(ulbuf[round_num + 1])
    output += put_uint32_be(ulbuf[round_num + 2])
    output += put_uint32_be(ulbuf[round_num + 3])
    return bytes(output)


def crack_round(roundFaultList, ref, last_round_key=None, verbose=1):
    """
    Try to crack a round key given faulty outputs.
    :param roundFaultList: the list with faulty ciphers
    :param ref: the correct cipher
    :param last_round_key: for decrypting the faulty cipher and correct cipher if not empty
    :return: the round key or None
    """
    if last_round_key is not None and len(last_round_key) != 0:
        """
            if last round key is not empty: require to decrypt the cipher by it
        """
        if verbose > 1:
            print(f"Rewinding round.")

        ref = rewind(ref, last_round_key)
        roundFaultList = [rewind(faulted, last_round_key) for faulted in roundFaultList]

    return crack_bytes(roundFaultList, ref, verbose)



def check(output, verbose=1, init=False, _intern={}):
    """
    Checks an output against a correct output.
    :param output: the first is correct output or faulty output
    :param init: if True, resets the internal as the given output
    :param _intern: the correct output
    :return: the index of fault bytes
    """
    if init:
        _intern.clear()
    # Initialization, the first time the normal encryption is executed
    if not _intern:
        _intern['goldenref'] = output
        if verbose > 2:
            print("FI: record golden ref")
        return (FaultStatus.NoFault, None)
    # correct cipher
    if output == _intern['goldenref']:
        if verbose > 2:
            print("FI: no impact")
        return (FaultStatus.NoFault, None)
    # Determine the fault byte by finding the byte iso-or
    diff = xor(output, _intern['goldenref'])
    # record the index of difference
    diffmap = [i for i in range(len(diff)) if diff[i] != 0]
    diffsum = len(diffmap)
    if diffsum == 5 or diffsum == 8 or diffsum == 9 or diffsum == 12 or diffsum == 13:
        if check_diff(diffmap, diffsum):
            if diffsum == 5:
                if verbose > 2:
                    print(
                        f"FI: good candidate for round N (({diffsum:{2}} bytes)): \t{output.hex()} {bytes(diff).hex()}")
                    print(bytes(diff).hex())
                index = [(3 - diffmap[diffsum - 1] % 4)]
                return (FaultStatus.MinorFault, index)
            elif diffsum == 9 or diffsum == 8:
                if verbose > 2:
                    print(
                        f"FI: good candidate for round N-1 (({diffsum:{2}} bytes)): \t{output.hex()} {bytes(diff).hex()}")
                index = [0, 1, 2, 3]
            elif diffsum == 13 or diffsum == 12:
                if verbose > 2:
                    print(
                        f"FI: good candidate for round N-2 (({diffsum:{2}} bytes)): \t{output.hex()} {bytes(diff).hex()}")
                index = [0, 1, 2, 3]
            else:
                return (FaultStatus.MinorFault, [])
            return (FaultStatus.GoodFault, index)
        else:
            if verbose > 2:
                print(f"FI: wrong candidate ({diffsum:{2}} bytes): \t\t{output.hex()} {bytes(diff).hex()}")
            return (FaultStatus.WrongFault, [])
    elif diffsum < 5:
        if verbose > 2:
            print(f"FI: too few impact ({diffsum:{2}} bytes): \t\t\t{output.hex()} {bytes(diff).hex()}")
        return (FaultStatus.MinorFault, [])
    else:
        if verbose > 2:
            print(f"FI: too much impact ({diffsum:{2}} bytes): \t\t{output.hex()} {bytes(diff).hex()}")
        return (FaultStatus.GoodFault, [])


def get_candidates(faultcipher, ref, index, verbose=1):
    """
    Get the key candidates according to IN_Table.
    """
    global IN_Table
    faultcipher = bytes2int(faultcipher)
    ref = bytes2int(ref)
    ref_slice = getSlices(ref)
    fault_slice = getSlices(faultcipher)
    delta_C = xor(ref_slice, fault_slice)[3]
    delta_B = l_inv(delta_C)
    A = ref_slice[0] ^ ref_slice[1] ^ ref_slice[2]
    A_star = fault_slice[0] ^ fault_slice[1] ^ fault_slice[2]
    result = [None] * 4
    for i in index:
        alpha = singleState(A ^ A_star, i)
        beta = singleState(delta_B, i)
        r = IN_Table[alpha][beta]
        if r:
            result[i] = [singleState(A, i) ^ x for x in r]
        else:
            if verbose > 2:
                print("Error: empty key candidate!")
    return result


def crack_bytes(roundFaultList, ref, verbose=1):
    """
    Try to crack a round key.
    :param roundFaultList: list of faulty outputs
    :param ref: the correct output
    :return: recovered round key
    """
    candidates = [None] * 4
    key = [None] * 4

    _, index = check(ref, verbose=verbose, init=True)
    for faultCipher in roundFaultList:
        # Check if key is recovered
        if key[0] != None and key[1] != None and key[2] != None and key[3] != None:
            break
        found_key = False
        _, index = check(faultCipher, verbose=verbose)
        if len(index) == 0:
            continue
        if verbose > 3:
            print(f"Key index at {index}")

        c = get_candidates(faultCipher, ref, index, verbose)
        for i in index:
            if key[i] is not None:
                continue
            if c[i] is None:
                continue
            if not candidates[i]:
                # initial candidate state
                candidates[i] = c[i]
            else:
                candidates[i] = list(set(candidates[i]) & set(c[i]))
                # get the exact key
                if (len(candidates[i]) == 1):
                    key[i] = candidates[i][0]
                    found_key = True
                elif len(candidates[i]) == 0:
                    # Something wrong happened we reset the candidate list.
                    candidates[i] = None

        if verbose > 1 and found_key:
            print("Round key bytes recovered:")
            print(''.join(["%02X" % x if x is not None else ".." for x in key]))
        if verbose > 2:
            print(f"Key candidates:{candidates}")
    # Check whether all key bytes have been recovered
    if None in key:
        return None
    return get_uint32_le(key)


def crack_file(filename, lastroundkeys=None, verbose=1):
    """
    Try to crack round keys
    :param filename: a filename of a file containing the corret cipher and fault ciphers
    :param lastroundkeys: a list of round keys for the rounds to rewind, as 32-bit int
    :return: recovered round keys
    """
    ref = None
    faults = []

    if lastroundkeys == None:
        lastroundkeys = []

    for line in open(filename):
        if len(line.split()) == 1:
            # only output available
            o = bytes.fromhex(line.strip())
            assert len(o) == blocksize
            if ref is None:
                ref = o
            else:
                faults.append(o)
        elif len(line.split()) == 2:
            i, o = line.split()
            i, o = bytes.fromhex(i), bytes.fromhex(o)
            assert len(i) == len(o) == blocksize
            if ref is None:
                ref = o
            else:
                faults.append(o)
        else:
            continue

    last_round_key = None
    round = 32 - len(lastroundkeys)
    while len(lastroundkeys) < 4:
        last_round_key = crack_round(faults, ref, lastroundkeys, verbose)
        if last_round_key == None:
            break
        else:
            print(f"Round key {round} found:")
            roundkey = ''.join(["%02X" % x for x in put_uint32_be(last_round_key)])
            print(roundkey)
            lastroundkeys.append(last_round_key)
            round -= 1

    return lastroundkeys
