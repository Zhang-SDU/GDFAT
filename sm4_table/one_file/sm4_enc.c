...
int main(int argc, u8 *argv[]) 
{
	...
	for (int round = 0; round < 32; round++)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			temp_list_u4[i] = xor_table[round][i][Xi1[i]][Xi2[i]];
		}
		for (int i = 0; i < 8; i++)
		{
			temp_list_u4[i] = xor_table[round][i + 8][temp_list_u4[i]][Xi3[i]];
		}
		u4_to_u32(temp_list_u4, &temp);
		u8 temp_list_u8[4] = { 0 };
		u32_to_u8(temp, temp_list_u8);

		u32 y0 = T[round][0][temp_list_u8[0]];
		u32 y1 = T[round][1][temp_list_u8[1]];
		u32 y2 = T[round][2][temp_list_u8[2]];
		u32 y3 = T[round][3][temp_list_u8[3]];
		...
		for (int i = 0; i < 8; i++)
		{
			temp_round_output1[i] = xor_table[round][16 + i][y0_u4[i]][y1_u4[i]];
		}
		for (int i = 0; i < 8; i++)
		{
			temp_round_output2[i] = xor_table[round][24 + i][y2_u4[i]][y3_u4[i]];
		}
		for (int i = 0; i < 8; i++)
		{
			temp_round_output1[i] = xor_table[round][32 + i][temp_round_output1[i]][temp_round_output2[i]];
		}
		for(int i=0;i<8;i++)
		{
			temp_round_output1[i] = xor_table[round][40 + i][temp_round_output1[i]][Xi0[i]];
		}
